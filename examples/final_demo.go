...
func main() {
	fmt.Println("开始最终综合演示...")

	// 创建场景
	scene := fauxgl.NewScene("Final Demo")
	...
	// 演示不同功能
	demonstrateAllFeatures(context, scene, camera, lights)

	fmt.Println("最终综合演示完成！")
}
...
// demonstrateAllFeatures 演示所有功能
func demonstrateAllFeatures(context *fauxgl.Context, scene *fauxgl.Scene, camera *fauxgl.OrbitCamera, lights []fauxgl.Light) {
	// 1. 基本渲染
	fmt.Println("1. 基本渲染...")
	renderBasic(context, scene, "final_demo_basic.png")

	// 2. 阴影映射
	fmt.Println("2. 阴影映射...")
	renderWithShadows(context, scene, lights[0], "final_demo_shadows.png")

	// 3. 后期处理
	fmt.Println("3. 后期处理...")
	renderWithPostProcessing(context, scene, "final_demo_postprocessing.png")

	// 4. 几何体细分
	fmt.Println("4. 几何体细分...")
	demonstrateSubdivision(context, scene, "final_demo_subdivision.png")
}
